...
function setQuote(){
    splashTextEl.textContent = QUOTES[Math.floor(Math.random() * QUOTES.length)].replace("$version", version);
}
...
export function updateGameDisplay(received, overwriteAll){
    ...
    eventLogEl.innerHTML = data.eventLog.length == 0 ? '<span class="inactive">Nothing here yet!</span>' : [...data.eventLog].reverse().map((text) => {
        text = escapeHTML(text);
        for(const [regex, className] of [
            [/turn \d+(-\d+)?/ig, "primary"],
            [/spell \d+/ig, "primary"],
            [/[0-9.]+ damage/ig, "neutral"],
            [/\d+ (ap|ability points?)/ig, "ap"],
            [RegExp(`\b(${players.map((player) => player.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join("|")})\b`, "g"), "secondary"]
        ]){
            text = text.replaceAll(regex, (match) => `<span class="${className}">${match}</span>`);
        }
        return text;
    }).join("\n");
    ...
    function item(dat, options){
        ...
        if(dat == null){
            ...
            for(const x of dat)
                if(x)
                    el.appendChild(item(x, { defaultTag: (options ?? {}).deepDefaultTag, deepDefaultTag: (options ?? {}).deeperDefaultTag }));
        }else{
            el = document.createElement(defaultTag);
            el.textContent = dat;
        }
        if(options){
            const { defaultTag: _, force: _2, deepDefaultTag: _3, deeperDefaultTag: _4, tooltip, className, ...rest } = options;
    ...
    function classTable(dat, className){
        return item(dat, { defaultTag: "table", deepDefaultTag: "tr", deeperDefaultTag: "td", className });
    }
    function newline(){
    ...
    yearEl.append("Year: ", classSpan(Math.floor((data.turn - 1) / 36) + 1, "primary"), ", Season: ", classSpan(season, type(data.season.type)))

    let weatherEl = document.createElement("div");
    weatherEl.className = "secondary";

    const temperature = data.temperature;
    weatherEl.append("Current Weather: ", classSpan(weather, type(data.weather.type)), ", ", classSpan(temperature + "°C", temperatureIndicator(temperature)));

    infobox("gameinfo", "Game Info", [
        [
            classDiv(`Turn: ${data.turn}-${data.subturn} (${players[data.subturn - 1]?.name}'s turn)`, "primary"),
            yearEl,
            weatherEl,
            classDiv("Next Event: Turn " + data.nextEvent, "lvl"),
            element("h4", "Disaster Info"),
            ...data.disaster ? [
                classDivInline(["Current disaster: ", classSpan({
                    "Air_Damage": "🏔 Blizzard",
                    "Earth_Damage": "🌎 Earthquake",
                    "Water_Damage": "🌊Tsunami",
                    "Fire_Damage": "🌋 Volcano Eruption",
                    "Thunder_Damage": "🗱 Lightning Storm",
                }[data.disaster.type], type(data.disaster.type))]),
                classDivInline(["All non-", classSpan(prettify(data.disaster.type), type(data.disaster.type)), " does nothing this turn!"]),
                classDivInline(["All ", classSpan(prettify(data.disaster.type), type(data.disaster.type)), " is increased by ", classSpan(data.disaster.intensity + "%", type(data.disaster.type))]),
            ] : [
                classDivInline(["Current disaster: ", classSpan("None", "inactive")]),
            ]
        ],
        [
            classDiv("Season/Weather Buffs:", "secondary"),
            list([
                classSpan(season + ": " + formatBuff(data.season, true), type(data.season.type)),
                classSpan(weather + ": " + formatBuff(data.weather, true), type(data.weather.type)),
                classSpan("Temperature: " + formatBuff({ intensity: data.temperature, type: "All Damage" }), temperatureIndicator(data.temperature)),
            ]),
            classDiv("Current Events:", "secondary"),
            list(data.curevent.map((buff) => classSpan(formatBuff(buff), (buff.intensity < 0 ^ buff.type == "Spell_Cost") ? "bad" : "ap"))),
        ]
    ]);
    ...
    infobox("lockout", "Lockout Challenges", [
        classSpan(["Next Reset: Turn ", classSpan(data.turn + 10 - (data.turn - 1) % 10, "primary")]),
        newline(),
        newline(),
    ...
    function playerHeader(player){
        ...
        if(player.name === "Victor") {
            ...
            td.setAttribute("tooltip", "This player has been censored under the groundhog act of 2024");
        }else{
            const arr = [player.name + " (", element("span", "❤".repeat(player.lives), { className: "hp" }), ")"];
            if(player.gameLevel)
                arr.push(classDiv("In Gulag", "bad"));
            td.append(...arr);
        }
        return td;
    }
    ...
    row("✷ Level", (player) => `${player.lvl} ➩ ${player.lvl + 1}\n[${player.lvlxp}/${player.nextlvl}]`, "lvl", null, `
        <h3>Level</h3>
        <p>Level up to gain ability points or increase weapon/spell tiers</p>
        <p>Deal damage to level up!</p>
    `);
    row("✷ Ability Points", (player) => player.abilityPoints, "ap", null, `
    ...
    `);
    row("❣ Health Regen", (player) => player.hpregen, "hp", null, `
        <h3>Health Regen</h3>
    ...
    `);
    row("⸎ Mana Regen", (player) => player.manaregen, "manarg", null, `
        <h3>Mana Regen</h3>
    ...
    for(const [i, [prettyBuff, buff, className, symbol]] of enumerate([
        ...
        for(const [j, size] of enumerate([
            ...
            if(potionImage){
                ...
                `;
                potionData[i + 1][j + 1] = item("", { defaultTag: "img", src: potionImage, tooltip })
            }
        }    
    }
