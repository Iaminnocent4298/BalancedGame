...
let mouseX, mouseY;
let mousePressed = false, mouseMoved = false;
...
let lastTime, deltaTime, timeElapsed = 0;
...
export function animationFrame(time){
    ...
    if(queuedResizeEl){
        ...
        radX = xRatio / min * 4 / 2; radY = yRatio / min * 3 / 2;
        
        transformationMatrix = new DOMMatrix();
        transformationMatrix.translateSelf(pixelWidth / 2, pixelHeight / 2);
        transformationMatrix.scaleSelf(pixelWidth / radX / 2, pixelHeight / radY / 2);
...
class Island {
    ...
    prepareUpdate(){
        for(const island of islands){
            if(island === this)
                continue;
            if(hypotInRad(this.x - island.x, this.y - island.y, ISLAND_REPEL_RADIUS)){
                let repelFac = (1 - Math.hypot(this.x - island.x, this.y - island.y) / ISLAND_REPEL_RADIUS) ** 6 * ISLAND_REPEL_FORCE * deltaTime;
                this.velX += (this.x - island.x) * repelFac;
                this.velY += (this.y - island.y) * repelFac;
            }
        }
        for(const island of this.connected){
            const dx = island.x - this.x, dy = island.y - this.y;
            const fac = Math.min(Math.hypot(dx, dy) - ISLAND_EDGE_SIZE, ISLAND_MAX_EDGE_DISTANCE) ** 2 * ISLAND_EDGE_FORCE * deltaTime;
            this.velX += dx * fac;
            this.velY += dy * fac;
        }
        ...
        const fac = Math.max(0, 1 - (Math.max(1, speed ** 3 * 9) * ISLAND_DAMPING * deltaTime));
        this.velX *= fac;
        this.velY *= fac;
    }
    update(){
...
function draw(force){   
    ...
    for(const island of islands){
        ...
        if(!draggedIsland && mouseX !== undefined && hypotInRad(mouseX - island.x, mouseY - island.y, ISLAND_RADIUS)){
    ...
    if(draggedIsland){
        ...
        ctx.fillText(hoveredIsland.id, hoveredIsland.x - 0.008, hoveredIsland.y + 0.062);

        if(mousePressed)
            draggedIsland = hoveredIsland;
    }else{
